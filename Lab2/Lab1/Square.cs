...
namespace Lab1 {
    public class Square : DisplayObject {
        ...
        public Square(Color color, int borderThickness, 
        ...
        public override void CheckCollision() {
            if (MainPoint.X + (BorderWidth >> 1) >= MainForm.PictureBoxWidth) {
                var point = MainPoint;
                point.X = MainForm.PictureBoxWidth - (BorderWidth >> 1) - BorderThickness - 4;
                MainPoint = point;
            }
            if (MainPoint.Y + (BorderWidth >> 1) >= MainForm.PictureBoxHeight) {
                var point = MainPoint;
                point.Y = MainForm.PictureBoxHeight - (BorderWidth >> 1) - BorderThickness - 4;
                MainPoint = point;
            }
            if (MainPoint.X - (BorderWidth >> 1) <= 0) {
                var point = MainPoint;
                point.X = 10 + (BorderWidth >> 1) + BorderThickness + 4;
                MainPoint = point;
            }
            if (MainPoint.Y - (BorderWidth >> 1) <= 0) {
                var point = MainPoint;
                point.Y = 10 + (BorderWidth >> 1) + BorderThickness + 4;
                MainPoint = point;
            }
        }
    }
