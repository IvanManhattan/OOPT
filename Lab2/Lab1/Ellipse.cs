...
namespace Lab1 {
    public class Ellipse : DisplayObject {
        ...
        public Ellipse(Color color, int borderThickness, 
        ...
        public override void CheckCollision() {
            if (MainPoint.X + _width >= MainForm.PictureBoxWidth) {
                var point = MainPoint;
                point.X = MainForm.PictureBoxWidth - _width - BorderThickness - 3;
                MainPoint = point;
            }
            if (MainPoint.Y + _height >= MainForm.PictureBoxHeight) {
                var point = MainPoint;
                point.Y = MainForm.PictureBoxHeight - _height - BorderThickness - 3;
                MainPoint = point;
            }
